...
const stepsAfterProcessing = steps.map(step => {
  step.when = {
    show() {
      doNotShowAgainBlock()
      selectStepBullets()
  ...
  step.modalOverlayOpeningPadding = 5;
  step.modalOverlayOpeningRadius = 5;
  step.arrow = false;
  // step.floatingUIOptions = {
  //   middleware: [window.FloatingUIDOM.offset({ mainAxis: 0, crossAxis: 12 })]
  // }

  return step
})

tour.addSteps(stepsAfterProcessing);
